...
export async function insertAgreement(dataAgreement){
   const db = await connect()
   const result = await db.query(`INSERT INTO agreement (id_contractor, location, location_reference, description, date_begin, date_end)
...
export async function updateAgreement(id, dataAgreement){
   const db = await connect()
   const result = await db.query(`UPDATE agreement SET location = ${dataAgreement.location}, location_reference = ${dataAgreement.location_reference}, description = ${dataAgreement.description} date_begin = ${dataAgreement.date_begin} date_end = ${dataAgreement.date_end} WHERE id_agreement = ${id}`)
   return result.recordsets
}
export async function deleteAgreement(id){
   const db = await connect()
   const result = await db.query(`DELETE FROM agreement WHERE id_agreement = ${id}`)
   return result.recordsets
}