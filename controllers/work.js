...
export async function updateWork(id, dataUser) {
   const db = await connect()
   console.log('ID', id, dataUser)

   try {
      const result = await db.query(
         `UPDATE works SET responsable = '${dataUser.responsable}', description = '${dataUser.description}', location='${dataUser.location}' WHERE id_work = ${id}`
      )
